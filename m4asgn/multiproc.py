...
# load and filter the NLTK word list once in the main process
...
# crack a password using multiprocessing
def crack_password(username, full_hash):
    print(f"Cracking password for {username} using {cpu_count()} cores...")
    start_time = time.time()

    # Create a pool of processes and map the word list
    with Pool(processes=cpu_count()) as pool:
        results = pool.map(check_word, [(word, full_hash) for word in word_list])

    # Filter out None results to find the cracked password
    cracked_passwords = [result for result in results if result]

    if cracked_passwords:
        elapsed_time = time.time() - start_time
        print(f"Password for {username} is '{cracked_passwords[0]}' (found in {elapsed_time:.2f} seconds)")
        return (username, cracked_passwords[0], elapsed_time)
    else:
        print(f"Password for {username} not found.")
        return (username, None, None)

# main block to safely handle multiprocessing
